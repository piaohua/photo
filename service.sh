...
binFile="$workDir/main-photo"
pidFile="$workDir/main-photo.pid"
error=""
...
stop() {
    if [[ -e $pidFile ]]; then
        ...
        pid=`ps aux | grep main-photo | grep -v grep | awk '{print $2}' | head -1`
    fi
...
rebuild() {
    ...
    go build -o main-photo -ldflags "-w -s" main.go
    GOPATH=$GOPATHOLD
}
